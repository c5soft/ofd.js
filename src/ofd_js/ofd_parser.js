...
import {Jbig2Image} from '../jbig2_js/jbig2';
import {parseSesSignature} from "./ses_signature_parser";
/**
 * 执行 OFD 解析三步流水线：解压 -> 获取文档根 -> 解析文档
 * @param {ArrayBufferOrFile} file - OFD 文件二进制数据
 * @returns {Promise<Object>} 解析完成的文档
 */
export const parseOfdSteps = async function (file) {
...
const getSealDocumentObj = async function (stampAnnot) {
...
const getJsonFromXmlContent = async function (zip, xmlName) {
    return new Promise((resolve, reject) => {
        ...
        zip.files[xmlName].async('string').then(function (content) {
            ...
            const parserInstance = new XMLParser(ops);
            let jsonObj = parserInstance.parse(content);
