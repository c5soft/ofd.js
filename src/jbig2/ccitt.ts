...
// Coding mode constants for 2D coding
const twoDimPass = 0;
const twoDimHoriz = 1;
const twoDimVert0 = 2;
...
interface CCITTFaxDecoderOptions {
  ...
  Columns?: number;
  Rows?: number;
  EndOfBlock?: boolean | null;
  BlackIs1?: boolean;
}

class CCITTFaxDecoder {
  private source: CCITTFaxDecoderSource;
  private eof: boolean;
  ...
  private _findTableCode(start: number, end: number, table: number[][], limit?: number): [boolean, number, boolean] {
  ...
  private _lookBits(n: number): number {
    let c: number;
  ...
  private _eatBits(n: number): void {
